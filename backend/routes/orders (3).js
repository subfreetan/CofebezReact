...
// === POST /api/orders/exchange-reward ===
// Échanger des points contre une récompense
router.post('/exchange-reward', authMiddleware, async (req, res) => {
  try {
    const { rewardName, rewardType, rewardImage, cost } = req.body;

    if (!rewardName || !cost || cost <= 0) {
      return res.status(400).json({ error: 'Données invalides' });
    }

    // Récupérer l'utilisateur et vérifier les points
    const { data: user, error: userError } = await supabase
      .from('users')
    ...
    // Déduire les points (le solde uniquement, pas les lifetime_points)
    ...
    // Créer une commande pour la récompense (payée avec points)
    const orderItems = [{
      name: rewardName,
      quantity: 1,
      price: 0, // Gratuit car payé avec des points
      isReward: true,
      rewardCost: cost,
    ...
    const { data: order, error: orderError } = await supabase
      ...
      .insert({
        ...
        points_spent: cost,
        points_earned: 0,
        note: `Récompense: ${rewardName} (${cost} pts)`
      })
      .select()
      .single();

    if (orderError) {
      console.error('Erreur création commande récompense:', orderError);
      // Rollback: remettre les points
      await supabase.from('users').update({ points: user.points }).eq('id', req.user.userId);
      return res.status(500).json({ error: 'Erreur lors de la création de la commande' });
    }
